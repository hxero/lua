...
local connect, wait, disconnect; do
	connect, wait = connection.Connect, connection.wait;
	local a = connect(connection, function() end)
...
if findFirstChild(coreGui, 'TurtleUiLib') then
	destroy(findFirstChild(coreGui, 'TurtleUiLib'))
	destroyed = true
end
		...
		connect(obj.InputBegan, function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				...
				con = connect(stepped, function()
					if isdragging then
						local delta = vector3New(mouse.X, mouse.Y, 0) - minitial;
...
local TurtleUiLib = instanceNew("ScreenGui")
...
function library:Destroy()
	...
	if keybindConnection then
		disconnect(keybindConnection)
	end
end
function library:Keybind(key)
	if keybindConnection then disconnect(keybindConnection) end

	keybindConnection = connect(uis.InputBegan, function(input, gp)
...
function library:Window(name) 
	...
	local UiWindow = instanceNew("Frame")
	...
	HeaderText.TextColor3 = fromRGB(47, 54, 64)
	...
	Minimise.Text = "—"
	Minimise.TextColor3 = fromRGB(0, 0, 0)
	...
	function functions:Button(name, callback)
		...
		Button.TextColor3 = fromRGB(245, 246, 250)
		Button.TextSize = 16.000
		Button.TextStrokeTransparency = 123.000
		Button.TextWrapped = true
		Button.Text = name
		connect(Button.MouseButton1Down, callback)
	...
	function functions:Label(text, color)
		...
		spawn(function()
				while wait() do
					local hue = tick() % 5 / 5
					Label.TextColor3 = fromHSV(hue, 1, 1)
				end
		end)
		else
	...
	function functions:Toggle(text, on, callback)
		...
		local ToggleDescription = instanceNew("TextLabel")
		local ToggleButton = instanceNew("TextButton")
		local ToggleFiller = instanceNew("Frame")
	...
	function functions:Box(text, callback)
		...
		TextBox.Text = ""
		TextBox.TextColor3 = fromRGB(245, 246, 250)
		TextBox.TextSize = 16.000
		TextBox.TextStrokeColor3 = fromRGB(245, 246, 250)
	...
	function functions:Slider(text, min, max, default, callback)
		...
		local SilderFiller = instanceNew("Frame")
		local Current = instanceNew("TextLabel")
		local Min = instanceNew("TextLabel")
	...
	function functions:Dropdown(text, buttons, callback)
		...
		local DownSign = instanceNew("TextLabel")
		...
		Dropdown.TextColor3 = fromRGB(245, 246, 250)
		...
		DropdownFrame.ScrollBarImageColor3 = fromRGB(220, 221, 225)
		table.insert(dropdowns, DropdownFrame)
		local dropFunctions = {}
		local canvasSize = 0
		function dropFunctions:Button(name)
			local name = name or ""
			local Button_2 = instanceNew("TextButton")
	...
	function functions:ColorPicker(name, default, callback)
		...
		local ColorPicker = instanceNew("TextButton")
		local PickerCorner = instanceNew("UICorner")
		local PickerDescription = instanceNew("TextLabel")
		local ColorPickerFrame = instanceNew("Frame")
		local ToggleRGB = instanceNew("TextButton")
		local ToggleFiller_2 = instanceNew("Frame")
		local TextLabel = instanceNew("TextLabel")
		local ClosePicker = instanceNew("TextButton")
		local Canvas = instanceNew("Frame")
		local CanvasGradient = instanceNew("UIGradient")
		local Cursor = instanceNew("ImageLabel")
		local Color = instanceNew("Frame")
		local ColorGradient = instanceNew("UIGradient")
		local ColorSlider = instanceNew("Frame")
		local Title = instanceNew("TextLabel")
		local UICorner = instanceNew("UICorner")
		local ColorCorner = instanceNew("UICorner")
		...
		connect(ColorPicker.MouseButton1Up, function()
		...
		connect(ClosePicker.MouseButton1Down, function()
			ColorPickerFrame.Visible = not ColorPickerFrame.Visible
		end)

		CanvasGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, fromRGB(255, 0, 0)), ColorSequenceKeypoint.new(1.00, fromRGB(255, 255, 255))}
		...
		if default and type(default) == "boolean" then
			...
			ColorPicker.BackgroundColor3 = default or fromRGB(0, 168, 255)
		end
		...
		connect(Color.InputEnded, function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			draggingColor = false
			end
		end)
		ColorGradient.Color = ColorSequence.new({
			...
			ColorSequenceKeypoint.new(0.66, fromRGB(0, 0, 255)), 
			ColorSequenceKeypoint.new(0.82, fromRGB(255, 0, 255)), 
